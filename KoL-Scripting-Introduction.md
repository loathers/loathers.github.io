...
LASS provides two very helpful packages for TypeScript projects. The first is [Libram](https://github.com/Loathing-Associates-Scripting-Society/libram), a resource chock-full of KoL-relevant functions. Libram has support for a great variety of IotMs, as well as general-purpose functions. One of libram's most useful and flexible features is its Macro system, which allows players to construct [BALLS](https://kol.coldfront.net/thekolwiki/index.php/Combat_Macros) Macros in their code, and either set it as an autoattack or submit it to `fight.php`.
